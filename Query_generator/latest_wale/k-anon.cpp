...
int main() {
 ...
 /* flag = 0;
  ...
  flag = 0;
	...
	string ans = "";
	ans += "SELECT * FROM K_Anonymous_Data WHERE Sex=\"Any\" AND Race=\"Person\" " ;
	if(age_input != -1){
	...
	if(edu_input != ""){
		sprintf(temp,"AND Education = \"%s\" ",edu_input.c_str());
		string g = temp;
		ans += g;
	}
	if(marital_input != ""){
		sprintf(temp,"AND MaritalStatus = \"%s\" ",marital_input.c_str());
		string g = temp;
		ans += g;
	}
	if(country_input != ""){
