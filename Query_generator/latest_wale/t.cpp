...
void pre() {
  ...
  // education
  edu.insert(mp("Bachelors","Undergraduate"));
  edu.insert(mp("Some-college","Undergraduate"));
...
int main() {
  ...
  string ans = "";
  ans += "SELECT * FROM t_close WHERE Sex=\"Any\" AND Race=\"Person\" " ;
  if(age_input != -1){
    if(age_input == 1){
    ...
    if(age_input == 3){
      sprintf(temp," AND Age = %d ",age_input);
    }
    string g = temp;
    ans += g;
  }
  if(edu_input != ""){
    if(edu_input == "School") {
      sprintf(temp,"AND ( Education = \"%s\" or Education =  \"School/Grad\"  or Education = \"School/UG\" ) ",edu_input.c_str());
    }
    if(edu_input == "Graduate"){
      sprintf(temp,"AND ( Education = \"%s\" or Education =  \"School/Grad\" ) ",edu_input.c_str());	
    }
    if(edu_input == "Undergraduate"){
      sprintf(temp,"AND ( Education = \"%s\" or Education =  \"School/UG\" ) ",edu_input.c_str());	
    }
    string g = temp;
    ans += g;
  }
